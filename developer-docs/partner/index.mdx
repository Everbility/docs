...
<Info>
Partner apps are provisioned manually. Email [support@everbility.com](mailto:support@everbility.com) to register as a partner and receive your `client_id` and `client_secret`.
</Info>
...
sequenceDiagram
    ...
    B->>E: Clinician signs in and writes the report in Everbility

    Note over P,E: Retrieve
    P->>E: GET /v1/partner/launch-sessions/{session_id}
    E-->>P: status: completed + document_url
...
<Steps>
  <Step title="Get credentials">
    Email [support@everbility.com](mailto:support@everbility.com) with your redirect URI to receive a `client_id` and `client_secret`.
  </Step>
  ...
  <Step title="Launch and retrieve">
    Create a [launch session](/developer-docs/partner/launch-sessions), hand the clinician the `launch_url`, and poll for the finished document.
  </Step>
</Steps>
